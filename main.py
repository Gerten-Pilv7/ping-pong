...
def mäng():
    ...
    pall_x = 420
    pall_y = 300
    suund_x = 2 if random.randint(0, 1) == 0.2 else -0.2
    suund_y = 1 if random.randint(0, 1) == 0.2 else -0.2
    def pall():
        global pall_x
        global pall_y
        global suund_x
        ...
        if pall_y <= 0 or pall_y >= y_ekraan:
            suund_y *= -1
        if int(pall_x) == int(mängija1x) + 12 and int(pall_y) in range(int(mängija1y), int(mängija1y) + 70):
            suund_x *= -1
        if int(pall_x) == int(x_ekraan - mängija2x) and int(pall_y) in range(int(mängija2y), int(mängija2y) + 70):
            suund_x *= -1
        pall_x += suund_x
        pall_y += suund_y
